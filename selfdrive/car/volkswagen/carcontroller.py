  ...
  def update(self, CC, CS, now_nanos):
    ...
    if self.frame % self.CCP.ACC_CONTROL_STEP == 0 and self.CP.openpilotLongitudinalControl:
      ...
      if self.CP.flags & VolkswagenFlags.MEB:
        ...
        acc_control = self.CCS.acc_control_value(CS.out.cruiseState.available, CS.out.accFaulted, CC.longActive or self.long_enabling, just_disabled, self.long_enabling)
        acc_hold_type = self.CCS.acc_hold_type(CS.out.cruiseState.available, CS.out.accFaulted, CC.longActive or self.long_enabling, just_disabled, starting,
                                               stopping, CS.esp_hold_confirmation)
        can_sends.extend(self.CCS.create_acc_accel_control(self.packer_pt, CANBUS.pt, CS.acc_type, CC.longActive or self.long_enabling, accel, acc_control,
